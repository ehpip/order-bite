...
export default function NewOrderSessionPage() {
  ...
  const [customItems, setCustomItems] = useState<{ name: string; price: number; description: string }[]>([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCustomStore) {
        ...
        const validCustomItems = customItems.filter((i) => i.name.trim() && i.price > 0);
        if (validCustomItems.length === 0) {
          alert('Please add at least one valid custom menu item with a price.');
          setLoading(false);
          return;
        }
        const snapRes = await db.createCustomSnapshot(customStoreName, validCustomItems);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Create Group Order Session</h1>
          <p className="text-xs text-slate-500">
            Set store, deadline, shipping cost, and generate a shareable order link.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-2xs space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Group Order Name *</label>
              <input
                type="text"
                required
                value={orderName}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Your Name (Host)</label>
              <input
                type="text"
                required
                value={hostName}
        ...
        <div className="space-y-4 pt-4 border-t border-slate-100">
          <div className="flex items-center justify-between">
            ...
            >
              {isCustomStore ? '← Select Existing Store' : '+ Create Custom Store & Menu'}
            </button>
          </div>

          {!isCustomStore ? (
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1.5">Choose Store Database</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {stores.map((store) => (
                  ...
                  >
                    {store.logo ? (
                      <img src={store.logo} alt={store.name} className="w-10 h-10 rounded-lg object-cover border" />
                    ) : (
                    ...
                    <div className="min-w-0">
                      <div className="font-bold text-sm text-slate-900 truncate">{store.name}</div>
                      <div className="text-xs text-slate-500 truncate">{store.description || 'Pre-loaded Menu'}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1">Custom Store Name *</label>
                <input
                  type="text"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-bold text-slate-800">Custom Menu Items</label>
                  <button
                    type="button"
                    onClick={handleAddCustomItem}
                    className="text-xs font-bold text-orange-600 flex items-center gap-1 cursor-pointer"
                  >
                  ...
                  {customItems.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-lg border border-slate-200">
                      <input
                        ...
                        className="flex-1 px-2 py-1 text-xs border rounded outline-hidden"
                      />
                      <input
                        type="number"
                        ...
                        className="w-28 px-2 py-1 text-xs border rounded outline-hidden"
                      />
        ...
        <div className="space-y-4 pt-4 border-t border-slate-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Ordering Closes In</label>
              <select
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Total Shipping Cost (Rp)</label>
              <input
                type="number"
                value={shippingCost}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 mb-1.5">Shipping Cost Allocation</label>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 font-normal mt-1">Divided evenly among members</span>
              </label>
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 font-normal mt-1">Based on member food total</span>
              </label>
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 font-normal mt-1">Members pay only for food</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2 pt-4 border-t border-slate-100">
          <label className="block text-xs font-bold text-slate-800">Payment Transfer Notes / Bank Account</label>
          <input
            type="text"
            value={paymentNotes}
            ...
            className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm outline-hidden focus:border-orange-600"
          />
          <p className="text-[11px] text-slate-500">This will be shown to members so they know where to pay you.</p>
        </div>
