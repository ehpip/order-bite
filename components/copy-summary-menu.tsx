...
import { Copy, Check, FileText, ShoppingBag, CreditCard, Bell } from 'lucide-react';
...
export default function CopySummaryMenu({ session, orders, storeName }: CopySummaryMenuProps) {
  ...
  const generateRestaurantSummary = () => {
    ...
    text += `🛵 Shipping: ${formatCurrency(session.shipping_cost)}\n`;
  ...
  const generateDeadlineReminder = () => {
    const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}/order/${session.share_code}` : `/order/${session.share_code}`;
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
      ...
      <p className="text-xs text-slate-500">
        Copy ready-to-send formatted text for WhatsApp groups, restaurant orders, or payment reminders.
      </p>
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                  isCopied ? 'bg-emerald-600 text-white' : 'bg-white border border-slate-200 text-orange-600'
                }`}
              >
                {isCopied ? <Check className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
              </div>
              <div className="min-w-0">
                <div className="text-xs font-bold leading-snug">{isCopied ? 'Copied to Clipboard!' : btn.label}</div>
                <div className="text-[11px] text-slate-500 truncate">{btn.sublabel}</div>
              </div>
            </button>
          );
        })}
